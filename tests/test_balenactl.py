...
from balena.balenactl import BalenaCtl

TEST_ENVIRONMENT = dict(
	BALENA_APP_ID='APP_ID',
	BALENA_SUPERVISOR_ADDRESS='http://localhost.test',
...
class BalenaCtlTestCase(unittest.TestCase):
	...
	def test_reboot(self):
		...
		mock_method.assert_called_once_with('http://localhost.test/v1/reboot?apikey=TEST_API_KEY', payload=None)

	@patch.dict('os.environ', TEST_ENVIRONMENT)
	def test_restart(self):
		with patch.object(BalenaCtl, '_perform_request', return_value=SuccessResponse()) as mock_method:
			BalenaCtl().onecmd("restart")
		mock_method.assert_called_once_with('http://localhost.test/v1/restart?apikey=TEST_API_KEY', payload={'apiKey': 'APP_ID'})

	@patch.dict('os.environ', TEST_ENVIRONMENT)
	def test_shutdown(self):
		with patch.object(BalenaCtl, '_perform_request', return_value=SuccessResponse()) as mock_method:
			BalenaCtl().onecmd("shutdown")
		mock_method.assert_called_once_with('http://localhost.test/v1/shutdown?apikey=TEST_API_KEY', payload=None)
